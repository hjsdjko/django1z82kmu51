...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"1px solid #C7C7C7","padding":"20px 10px 0","margin":"0 0 20px","alignItems":"center","background":"#fff","flexDirection":"row","display":"flex","width":"100%","height":"auto","order":"1"}'>
      <el-form-item :style='{"flexWrap":"wrap","margin":"0 0 20px","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 5px 0","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>教室编号：</div>
        <el-input v-model="formSearch.jiaoshibianhao" placeholder="教室编号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"flexWrap":"wrap","margin":"0 0 20px","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 5px 0","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>教室名称：</div>
        <el-input v-model="formSearch.jiaoshimingcheng" placeholder="教室名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"flexWrap":"wrap","margin":"0 0 20px","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 5px 0","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>教室位置：</div>
        <el-input v-model="formSearch.jiaoshiweizhi" placeholder="教室位置" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	...
	<div class="list" :style='{"border":"1px solid #C7C7C7","width":"auto","margin":"0","background":"#fff","flex":"1","order":"3"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"20px 10px 0","background":"#fff","height":"auto"}'>
			<div :style='{"border":"1px solid #E0E0E0","padding":"8px","margin":"0 1% 20px","background":"#ffff","display":"inline-block","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"padding":"0 10px","lineHeight":"1.5","fontSize":"14px","color":"#000","textAlign":"center"}' class="name ">{{item.jiaoshibianhao}}</div>
				<div :style='{"padding":"0 10px","lineHeight":"1.5","fontSize":"14px","color":"#000","textAlign":"center"}' class="name ">{{item.jiaoshimingcheng}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.jiaoshibianhao != '') searchWhere.jiaoshibianhao = '%' + this.formSearch.jiaoshibianhao + '%';
        if (this.formSearch.jiaoshimingcheng != '') searchWhere.jiaoshimingcheng = '%' + this.formSearch.jiaoshimingcheng + '%';
        if (this.formSearch.jiaoshiweizhi != '') searchWhere.jiaoshiweizhi = '%' + this.formSearch.jiaoshiweizhi + '%';
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`jiaoshixinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/jiaoshixinxiDetail', query: params});
      },
